...
terraform {
  ...
  # todo: revert this when breaking change is fixed in terraform-aws-ecs
...
inputs = {
  ...
  container_definitions = {
    ...
    (local.name) = {
      ...
      secrets = [
        ...
        {
          name      = "RDS_TODO_DB_NAME"
          valueFrom = format("%s/%s", local.param_base_path, "common/rds/todo-db-name")
        },
        {
          name      = "RDS_TODO_DB_USERNAME"
          valueFrom = format("%s/%s", local.param_base_path, "common/rds/todo-db-username")
        },
        {
          name      = "RDS_TODO_DB_PASSWORD"
          valueFrom = format("%s/%s", local.param_base_path, "common/rds/todo-db-password")
        }
      ]
  ...
  service_registries = {
    registry_arn = dependency.service-discovery.outputs.todo_service_discovery_arn
  }

  load_balancer = {
    service = {
      target_group_arn = dependency.nlb.outputs.target_groups["todo_service"].arn
