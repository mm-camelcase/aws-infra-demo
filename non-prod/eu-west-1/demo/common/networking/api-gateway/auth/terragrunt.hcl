...
locals {
  ...
  vpc_link_name = format("%s-%s-%s", local.env_config.locals.env, local.acc_config.locals.resource_prefix, "shared")
  api_domain    = "auth.camelcase.club"
}
...
inputs = {
  ...
  routes = {
    ...
    "$default" = {
      integration = {
        connection_type      = "VPC_LINK"
        uri                  = dependency.nlb.outputs.listeners["tcp_keycloak_service"].arn # Default to ECS NLB
        type                 = "HTTP_PROXY"
        method               = "ANY"
        timeout_milliseconds = 12000
        vpc_link_key         = "demo-vpc"
